...
export const FileUpload = ({
  ...
  // Sync the external files prop with the internal state
  useEffect(() => {
    if (files.length > 0) {
  ...
  const handleFileChange = (newFiles: File[]) => {
    if (newFiles.length > 0) {
      ...
      // Replace existing files instead of adding
  ...
  const filesToDisplay = uploadedFiles.length > 0 ? uploadedFiles : files;

  const handleSaveButtonClick = (e: React.MouseEvent) => {
    e.stopPropagation(); // Prevent event from bubbling up to the parent div
    if (onSave && filesToDisplay.length > 0) {
      onSave(filesToDisplay[0]);
    }
  };
          ...
          <div className="relative w-full mt-10 max-w-xl mx-auto">
            {error && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="mb-4 p-3 bg-red-900 text-white rounded-md text-sm"
              >
                {error}
              </motion.div>
            )}
            ...
            {showSaveButton && filesToDisplay.length > 0 && (
              <ShinyButton
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="mt-6 px-6 py-2 text-white border-2 rounded-md font-medium transition-colors duration-200 w-full max-w-xs mx-auto block relative z-50"
